	...
	<head>
		...
		function hostSumDetail(field){
			...
			if("images/minus.jpg"==imageSrc){
				...
				if(operated=="no"){
					...
					var tableTitle="<tr role=\"head\" ><th ></th><th sort=\"true\" >序号</th><th  sort=\"true\">学校名称</th><th  sort=\"true\">类型</th><th sort=\"true\" class='tr' >被感染IP数</th><th class='tr' sort=\"true\" >被感染链接数</th><th sort=\"true\" class=\"tr\" >被攻击次数</th><th sort=\"true\" class=\"tr\" >最近一次攻击时间</th><th sort=\"true\" class=\"tr\" >未处理</th></tr>";
					//ajax得到详细数据
			...
			$.ajax({
				...
				success: function(backData){
					...
					if(backData.length == 0){
						...
						var department = '${currentUser.department}';
						if("技术开发部" != department){
							result+="<tr><td>合计</td><td colspan='3'></td><td class='tr'>" + '${sumIpWS}' + "</td>"+
							"<td class='tr'>" + '${sumUrlWS}' +"</td>" +
							"<td class='tr'>" + '${sumWS}' +"</td>" +
							"<td></td>" + 
							"<td class='tr'>" + '${sumErrWS}' +"</td>" + 
							"</tr>";
						}
					}
				  }
		...
		//	var threedetailTable = document.getElementById("three_" +id);
			...
			if("images/minus.jpg"==imageSrc){
				...
				if(operated=="no"){
					var tableTitle="<tr role=\"head\"><th sort=\"true\" width='5%'>序号</th><th sort=\"true\" >被控地址</th><th sort=\"true\" class='tr' >被控域名</th><th sort=\"true\" class='tr' >被控链接</th><th sort=\"true\" class='tr' >最后一次攻击时间</th><th class='tr' sort=\"true\" >攻击次数</th><th class='tr' sort=\"true\">操作</th></tr>";
					//ajax得到详细数据
			...
			$.ajax({
				...
				success: function(backData){
					...
					for(var i=0;i<backData.length;i++){
						...
						result+="<td class='tr'>"+backData[i].domain+"</td>";
						//result+="<td class='tr'>"+backData[i].url +"</td>";
		...
		<div class="main mt10">
			<table  class="main-table" id="myTable">
      			...
      			<tr role="head">
					...
					<th sort="true" class="tr" >被感染链接数</th> 
			    ...
			    <c:forEach var="model" varStatus="cur" items="${requestScope.webshellList}" >
